...
FastAPI sidecar — ה-seam שאליו יתחבר ה-UI. Endpoints:
- `POST /auth/register` — הרשמה, מחזיר JWT (ציבורי)
- `POST /auth/login` — כניסה, מחזיר JWT (ציבורי)
- `GET /agents` — רשימת אג'נטים (דורש JWT)
- `POST /run` — הרצת אג'נט + רישום שימוש (דורש JWT)
- `WS /ws/run?token=...` — stream בזמן אמת (דורש JWT כ-query param)

**תיגעי בו** כשתוסיפי endpoints או תשני את לוגיקת האימות.
...
## Auth — `core/auth/` [ממומש]
מודול אימות משתמשים מלא, מחובר ל-MongoDB Atlas.
- `mongo.py` — singleton client ל-Atlas; `users_collection()` עם unique index על email.
- `models.py` — סכמות: `UserRegister`, `UserLogin`, `TokenResponse`.
- `service.py` — `register_user` (bcrypt hash + insert), `login_user` (verify + רישום login event),
  `record_usage` (push שימוש לאג'נט), `create_token`/`decode_token` (JWT, תוקף שבוע).

**נתונים שנשמרים ב-MongoDB לכל משתמש:**
```json
{ "email": "...", "password_hash": "...", "created_at": "...",
  "logins": [{"timestamp": "...", "ip": "..."}],
  "usage":  [{"timestamp": "...", "agent": "..."}] }
```
**תיגעי ב-service.py** כשתרצי להוסיף שדות סטטיסטיקה.
...
- `pyproject.toml` — תלויות (langgraph, httpx, pydantic, pyyaml, fastapi, uvicorn,
  python-dotenv, truststore, pypdf, **pymongo, python-jose, passlib**). התקנה: `pip install -e .`
- `.env.example` — תבנית למפתחות: `OPENROUTER_API_KEY`, `MONGODB_URI`, `MONGODB_DB`, `JWT_SECRET`.
